...
##Usage: $ python3 GAMMA-S.py my_scaffolds.fasta gene_db.fasta My_output
...
def Indel_Line(PSL_line, genome_gene, gene, verbose):
    ...
    Out = List1[13] + '\t' + List1[9] + '\t' + Start + '\t' + Stop + '\t' + Type + '\t' + Description + '\t' + str(BP_Changes) + '\t' + str(Transversions) + '\t' + Indels_All + '\t' + Percent_Bases + '\t' + Percent_Match + '\t' + Percent_Length + '\t' + List1[14] + '\t' + List1[8]
    return Out
...
##                    if Add == 0:
##                        break
                if float(List2[13]) > float(List1[13]):
                    Add = 0
                    break
                elif float(List2[13]) == float(List1[13]):
                            ...
                            if int(List2[6]) < int(List1[6]):
                                ...
                                if int(List2[7]) < int(List1[7]):
                                    ...
                                    if Order_List[0] != items:
                                        Add = 0
                                        break
        if Add == 1 and Star == 0 and float(List1[14]) > 0.2:
            Output_List.append(items)
        elif Add == 1 and Star == 1 and float(List1[14]) > 0.2:
            List1[0] = List1[0] + '‡'
            Out = '\t'.join(List1)
            Output_List.append(Out)
    return(Output_List)

def GAMMA_S_Output(PSL, genome_fasta, gene_fasta, out_file, verbose):
    Lines = Line_Lister(PSL, genome_fasta, gene_fasta, verbose)
    Contigs = Contig_Overlaps(Lines)
    Out = open(out_file, 'w')
    Out.write('Gene\tContig\tStart\tStop\tMatch_Type\tDescription\tMismatches\tTransversions\tInsertions\tInsertion BP\tDeletions\tDeletions BP\tUnweighted Match %\tMatch %\tLength %\tTarget Length\tStrand\n')
    for items in Contigs:
        Best = Best_List(items)
        for entry in Best:
            Out.write(entry + '\n')
    Out.close()
...
def GAMMA_S_Out(fasta, gene_db, output, identity, verbose):
    subprocess.call('blat' + ' ' + gene_db + ' '  + fasta + ' -noHead -minIdentity=' + str(identity) + ' ' + output + '.psl', shell=True)
    GAMMA_S_Output(output + '.psl', fasta, gene_db, output + '.gamma', verbose)
...
def main():
    parser = argparse.ArgumentParser(description="""This scripts makes sequence match calls from matches in an assembly using a sequence database""")
...
##                        help="write gene matches as gff file")
    ...
    if args.all:
        ...
        GAMMA_S_Out(args.input_fasta, args.database, args.output,  args.percent_identity, Verbose)
##    if args.fasta:
